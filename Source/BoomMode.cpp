...
bool BoomMode::Render()
{
	...
	if (IsRaceStarted()) {
		...
		if (isPlayerLast) {
			...
			string extra0 = (explosionTime - (float)timeToExplode.ReadSec() < 10 ? "0" : "");
			string countdown("00:"+ extra0 + App->localization->FormatNumber(explosionTime - (float)timeToExplode.ReadSec(), 0));
		...
		{
			App->renderer->DrawText(App->localization->FormatNumber(explosionTime - (float)timeToExplode.ReadSec(), 0).c_str(), { SCREEN_WIDTH/2- MeasureTextEx(App->assetLoader->agencyB, App->localization->FormatNumber(explosionTime - (float)timeToExplode.ReadSec(), 0).c_str(), 30, 0).x, 20 }, { 0, 0 }, App->assetLoader->agencyB, 80, 0, WHITE);
		}
